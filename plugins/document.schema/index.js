...
var SchemaPlugin = (function (_super) {
    ...
    SchemaPlugin.prototype.directive = function (directive) {
        ...
        return this.html.line(this.html.keyword('directive') + ' ' +
            ...
            directive.locations.map(function (location) { return _this.html.keyword(location); }).join(' | '));
    };
    SchemaPlugin.prototype.enum = function (type) {
        var _this = this;
        var reduceEnumValues = function (lines, enumValue) { return lines
            .concat([''], _this.description(enumValue.description), [_this.html.property(enumValue.name) + _this.deprecated(enumValue)]); };
    ...
    SchemaPlugin.prototype.field = function (field) {
        ...
        ].concat(this.argumentsMultiline(field).map(function (l) { return _this.html.tab(l); }), [
    ...
    SchemaPlugin.prototype.inputValues = function (inputValues) {
        var _this = this;
        return inputValues
            .map(function (inputValue) { return _this.html.line(_this.html.tab(_this.inputValue(inputValue))); })
            .join('');
    };
        ...
        if (schema.queryType)
            definition += this.html.line() +
                this.description(schema.queryType.description)
                    .map(function (line) { return _this.html.line(_this.html.tab(line)); })
                    .join('') +
                this.html.line(this.html.tab(this.html.property('query') + ': ' + this.html.useIdentifier(schema.queryType, this.url(schema.queryType))));
        if (schema.mutationType)
            definition += this.html.line() +
                this.description(schema.mutationType.description)
                    .map(function (line) { return _this.html.line(_this.html.tab(line)); })
                    .join('') +
                this.html.line(this.html.tab(this.html.property('mutation') + ': ' + this.html.useIdentifier(schema.mutationType, this.url(schema.mutationType))));
        if (schema.subscriptionType)
            definition += this.html.line() +
                this.description(schema.subscriptionType.description)
                    .map(function (line) { return _this.html.line(_this.html.tab(line)); })
                    .join('') +
                this.html.line(this.html.tab(this.html.property('subscription') + ': ' + this.html.useIdentifier(schema.subscriptionType, this.url(schema.subscriptionType))));
        definition += this.html.line('}');
        return definition;
        /* .concat(
         schema.directives
    ...
    SchemaPlugin.prototype.union = function (type) {
        var _this = this;
        return this.html.line(this.html.keyword('union') + ' ' + this.html.identifier(type) + ' = ' +
            type.possibleTypes
                .map(function (type) { return _this.html.useIdentifier(type, _this.url(type)); })
                .join(' | '));
    };
    return SchemaPlugin;
}(utility_1.Plugin));
